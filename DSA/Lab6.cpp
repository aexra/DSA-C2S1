...
void Lab6::t1()
{
	...
	cout << "Queue 1: " << q1;
	cout << "Queue 2: " << q2;
	cout << "\nѕримен€ем перемещение элементов..." << endl;
	q2 += q1;
	cout << "Queue 2: " << q2;
	cout << "\nјдреса новых начала и конца:" << endl << "Ќачало: " << q2.__GetHeadPtr__() << endl << " онец: " << q2.__GetTailPtr__();
}
...
void List<T>::remove(size_t i)
{
	...
	while (j != i)
	{
		if (!tmp)
		{
			error("Ёлемент с таким индексом не найден!\n\n");
			exit(-1);
...
size_t List<T>::find(T value)
{
	...
	error("Ёлемент с таким значением не найден!\n\n");
	exit(-1);
}
